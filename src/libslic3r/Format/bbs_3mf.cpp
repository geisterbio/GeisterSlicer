...
// 2 : Volumes' matrices and source data added to Metadata/Slic3r_PE_model.config file, meshes transformed back to their coordinate system
// on loading. WARNING !! -> the version number has been rolled back to 1
...
const char*        BBS_VALID_OBJECT_TYPES[]     = {"model", "other"};

const char* BBS_INVALID_OBJECT_TYPES[] = {"solidsupport", "support", "surface"};

template<typename T> struct hex_wrap
{
    T t;
};
...
basic_ostream<_Elem, _Traits>& operator<<(basic_ostream<_Elem, _Traits>& ostr, const hex_wrap<_Arg>& wrap)
...
class _BBS_3MF_Importer : public _BBS_3MF_Base
{
    ...
    struct Component
    {
        ...
        explicit Component(Id object_id) : object_id(object_id), transform(Transform3d::Identity()) {}

        Component(Id object_id, const Transform3d& transform) : object_id(object_id), transform(transform) {}
    };
    ...
    struct Instance
    {
        ...
        Instance(ModelInstance* instance, const Transform3d& transform) : instance(instance), transform(transform) {}
    };
    ...
    struct Metadata
    {
        ...
        Metadata(const std::string& key, const std::string& value) : key(key), value(value) {}
    };
    ...
    struct ObjectMetadata
    {
        struct VolumeMetadata
        {
            ...
            VolumeMetadata(unsigned int first_triangle_id, unsigned int last_triangle_id, ModelVolumeType type = ModelVolumeType::MODEL_PART)
                : first_triangle_id(first_triangle_id), last_triangle_id(last_triangle_id), part_type(type), subobject_id(-1)
            {}

            VolumeMetadata(int sub_id, ModelVolumeType type = ModelVolumeType::MODEL_PART)
                : subobject_id(sub_id), part_type(type), first_triangle_id(0), last_triangle_id(0)
            {}
        };
    ...
    struct ObjectImporter
    {
        ...
        ~ObjectImporter() { _destroy_object_xml_parser(); }
        ...
        bool extract_object_model()
        {
            ...
            if (!top_importer->_extract_from_archive(
                    archive, object_path,
                    [this](mz_zip_archive& archive, const mz_zip_archive_file_stat& stat) {
            ...
            if (obj_parse_error) {
                ...
                BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << ":" << __LINE__
                                         << boost::format(", Found error while extrace object %1%\n") % object_path;
                return false;
            }
            return true;
        }
...
public:
    ...
    bool load_gcode_3mf_from_stream(
        std::istream& data, Model& model, PlateDataPtrs& plate_data_list, DynamicPrintConfig& config, Semver& file_version);
...
private:
    ...
    bool _extract_from_archive(mz_zip_archive&    archive,
                               std::string const& path,
                               std::function<bool(mz_zip_archive& archive, const mz_zip_archive_file_stat& stat)>,
                               bool restore = false);
    bool _extract_xml_from_archive(mz_zip_archive&         archive,
                                   std::string const&      path,
    ...
    void _extract_cut_information_from_archive(mz_zip_archive&                 archive,
                                               const mz_zip_archive_file_stat& stat,
                                               ConfigSubstitutionContext&      config_substitutions);
    void _extract_layer_heights_profile_config_from_archive(mz_zip_archive& archive, const mz_zip_archive_file_stat& stat);
    void _extract_layer_config_ranges_from_archive(mz_zip_archive&                 archive,
                                                   const mz_zip_archive_file_stat& stat,
                                                   ConfigSubstitutionContext&      config_substitutions);
    ...
    void _extract_print_config_from_archive(mz_zip_archive&                 archive,
                                            const mz_zip_archive_file_stat& stat,
                                            DynamicPrintConfig&             config,
                                            ConfigSubstitutionContext&      subs_context,
                                            const std::string&              archive_filename);
    // BBS: add project config file logic
    void _extract_project_config_from_archive(mz_zip_archive&                 archive,
                                              const mz_zip_archive_file_stat& stat,
                                              DynamicPrintConfig&             config,
                                              ConfigSubstitutionContext&      subs_context,
                                              Model&                          model);
    // BBS: extract project embedded presets
    void _extract_project_embedded_presets_from_archive(mz_zip_archive&                 archive,
                                                        const mz_zip_archive_file_stat& stat,
                                                        std::vector<Preset*>&           project_presets,
                                                        Model&                          model,
                                                        Preset::Type                    type,
    ...
    bool _create_object_instance(
        std::string const& path, int object_id, const Transform3d& transform, const bool printable, unsigned int recur_counter);
    ...
    bool _generate_volumes_new(ModelObject&                              object,
                               const std::vector<Component>&             sub_objects,
...
// BBS: add plate data related logic
//  add backup & restore logic
bool _BBS_3MF_Importer::load_model_from_file(const std::string&         filename,
                                             Model&                     model,
                                             PlateDataPtrs&             plate_data_list,
                                             std::vector<Preset*>&      project_presets,
                                             DynamicPrintConfig&        config,
                                             ...
                                             int                        plate_id)
{
    ...
    if (m_load_restore) {
        ...
        save_string_file(model.get_backup_path() + "/lock.txt", boost::lexical_cast<std::string>(get_current_pid()));
    } else {
...
bool _BBS_3MF_Importer::get_thumbnail(const std::string& filename, std::string& data)
{
    ...
    if (m_thumbnail_middle.empty())
        m_thumbnail_middle = m_thumbnail_path;
    if (!m_thumbnail_middle.empty()) {
...
bool _BBS_3MF_Importer::load_gcode_3mf_from_stream(
    std::istream& data, Model& model, PlateDataPtrs& plate_data_list, DynamicPrintConfig& config, Semver& file_version)
{
    mz_zip_archive archive;
    mz_zip_zero_struct(&archive);
    ...
    if (m_thumbnail_small.empty())
        m_thumbnail_small = m_thumbnail_path;
    ...
    for (mz_uint i = 0; i < num_entries; ++i) {
        if (mz_zip_reader_file_stat(&archive, i, &stat)) {
            ...
            BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ":" << __LINE__
                                    << boost::format("extract %1%th file %2%, total=%3%\n") % (i + 1) % name % num_entries;
    ...
    while (it != m_plater_data.end()) {
        ...
        if (!plate->thumbnail_file.empty())
            _extract_from_archive(archive, plate->thumbnail_file,
                                  [&pixels = plate_data_list[it->first - 1]->plate_thumbnail.pixels](auto&       archive,
                                                                                                     auto const& stat) -> bool {
        ...
        BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ":" << __LINE__
                                << boost::format(", plate %1%, thumbnail_file=%2%, no_light_thumbnail_file=%3%") % it->first %
                                       plate->thumbnail_file % plate->no_light_thumbnail_file;
        BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ":" << __LINE__
                                << boost::format(", top_thumbnail_file=%1%, pick_thumbnail_file=%2%") % plate->top_file % plate->pick_file;
        it++;
    }
...
// BBS: add plate data related logic
bool _BBS_3MF_Importer::_load_model_from_file(std::string                filename,
                                              Model&                     model,
                                              PlateDataPtrs&             plate_data_list,
                                              std::vector<Preset*>&      project_presets,
                                              DynamicPrintConfig&        config,
                                              ...
                                              int                        plate_id)
{
    ...
    // BBS progress point
    BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ":" << __LINE__
                            << boost::format("import 3mf IMPORT_STAGE_OPEN, m_load_restore=%1%\n") % m_load_restore;
    if (proFn) {
    ...
    m_model->model_info->load(model_info);
    if (!m_thumbnail_small.empty())
        m_model->model_info->metadata_items.emplace("Thumbnail_Small", m_thumbnail_small);
    if (!m_thumbnail_middle.empty())
        m_model->model_info->metadata_items.emplace("Thumbnail_Middle", m_thumbnail_middle);
    ...
    for (mz_uint i = 0; i < num_entries; ++i) {
        if (mz_zip_reader_file_stat(&archive, i, &stat)) {
            ...
            BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ":" << __LINE__
                                    << boost::format("extract %1%th file %2%, total=%3%") % (i + 1) % name % num_entries;
            ...
            } else if (!dont_load_config && boost::algorithm::istarts_with(name, METADATA_DIR) &&
                       boost::algorithm::iends_with(name, GCODE_EXTENSION)) {
                // load gcode files
                _extract_file_from_archive(archive, stat);
            } else if (!dont_load_config && boost::algorithm::istarts_with(name, METADATA_DIR) &&
                       boost::algorithm::iends_with(name, THUMBNAIL_EXTENSION)) {
                // BBS parsing pattern thumbnail and plate thumbnails
                _extract_file_from_archive(archive, stat);
            } else if (!dont_load_config && boost::algorithm::istarts_with(name, METADATA_DIR) &&
                       boost::algorithm::iends_with(name, CALIBRATION_INFO_EXTENSION)) {
                // BBS parsing pattern config files
                _extract_file_from_archive(archive, stat);
            } else {
                BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ":" << __LINE__
                                        << boost::format(", %1% skipped, already parsed or a directory or not supported\n") % name;
            }
        }
    }
    ...
    BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ":" << __LINE__
                            << boost::format(", process group colors, size %1%\n") % m_group_id_to_color.size();
    ...
    BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ":" << __LINE__
                            << boost::format(", begin to assemble objects, size %1%\n") % m_objects.size();
    ...
    for (const IdToModelObjectMap::value_type& object : m_objects) {
        ...
        if (current_plate_data) {
            ...
            if (it == current_plate_data->obj_inst_map.end()) {
                ...
                BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ":" << __LINE__
                                        << boost::format(", could not find object %1% in plate %2%, skip it\n") % object.first.second %
                                               plate_id;
                continue;
            }
        }
        ...
        if (cut_object_info != m_cut_object_infos.end()) {
            ...
            for (auto connector : cut_object_info->second.connectors) {
                ...
                model_object->volumes[connector.volume_id]->cut_info = ModelVolume::CutInfo(CutConnectorType(connector.type),
                                                                                            connector.r_tolerance, connector.h_tolerance,
                                                                                            true);
            }
        }
    }
    ...
    // BBS progress point
    BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ":" << __LINE__
                            << boost::format("import 3mf IMPORT_STAGE_LOADING_PLATES, m_plater_data size %1%, m_backup_path %2%\n") %
    ...
    while (it != m_plater_data.end()) {
        ...
        plate_data_list[it->first - 1]->gcode_file              = (m_load_restore || it->second->gcode_file.empty()) ?
                                                                      it->second->gcode_file :
                                                                      m_backup_path + "/" + it->second->gcode_file;
        ...
        plate_data_list[it->first - 1]->thumbnail_file          = (m_load_restore || it->second->thumbnail_file.empty()) ?
                                                                      it->second->thumbnail_file :
                                                                      m_backup_path + "/" + it->second->thumbnail_file;
        // plate_data_list[it->first-1]->pattern_file = (m_load_restore || it->second->pattern_file.empty()) ? it->second->pattern_file :
        // m_backup_path + "/" + it->second->pattern_file;
        plate_data_list[it->first - 1]->no_light_thumbnail_file = (m_load_restore || it->second->no_light_thumbnail_file.empty()) ?
                                                                      it->second->no_light_thumbnail_file :
                                                                      m_backup_path + "/" + it->second->no_light_thumbnail_file;
        plate_data_list[it->first - 1]->top_file                = (m_load_restore || it->second->top_file.empty()) ?
                                                                      it->second->top_file :
                                                                      m_backup_path + "/" + it->second->top_file;
        plate_data_list[it->first - 1]->pick_file               = (m_load_restore || it->second->pick_file.empty()) ?
                                                                      it->second->pick_file :
        ...
        plate_data_list[it->first - 1]->pattern_bbox_file       = (m_load_restore || it->second->pattern_bbox_file.empty()) ?
                                                                      it->second->pattern_bbox_file :
                                                                      m_backup_path + "/" + it->second->pattern_bbox_file;
        plate_data_list[it->first - 1]->config                  = it->second->config;

        current_plate_data = plate_data_list[it->first - 1];
        BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ":" << __LINE__
                                << boost::format(", plate %1%, thumbnail_file=%2%, no_light_thumbnail_file=%3%") % it->first %
                                       plate_data_list[it->first - 1]->thumbnail_file %
                                       plate_data_list[it->first - 1]->no_light_thumbnail_file;
        BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ":" << __LINE__
                                << boost::format(", top_thumbnail_file=%1%, pick_thumbnail_file=%2%") %
                                       plate_data_list[it->first - 1]->top_file % plate_data_list[it->first - 1]->pick_file;
        it++;
        ...
        while (map_it != current_plate_data->obj_inst_map.end()) {
            ...
            if (index_iter == m_index_paths.end()) {
                BOOST_LOG_TRIVIAL(warning) << __FUNCTION__ << ":" << __LINE__
                                           << boost::format(", can not find object from plate's obj_map, id=%1%, skip this object") %
                                                  obj_id;
                map_it++;
                continue;
            }
            ...
            if (object_item == m_objects.end()) {
                BOOST_LOG_TRIVIAL(warning) << __FUNCTION__ << ":" << __LINE__
                                           << boost::format(", can not find object from plate's obj_map, ID <%1%, %2%>, skip this object") %
                                                  index_iter->second % index_iter->first;
                map_it++;
                continue;
            }
...
bool _BBS_3MF_Importer::_extract_from_archive(mz_zip_archive&                                                                    archive,
                                              std::string const&                                                                 path,
                                              std::function<bool(mz_zip_archive& archive, const mz_zip_archive_file_stat& stat)> extract,
                                              bool                                                                               restore)
{
    mz_uint                  num_entries = mz_zip_reader_get_num_files(&archive);
    mz_zip_archive_file_stat stat;
    ...
    if (index < 0 || !mz_zip_reader_file_stat(&archive, index, &stat)) {
        if (restore) {
            ...
            if (!m_origin_file.empty())
                paths.push_back(m_origin_file);
...
bool _BBS_3MF_Importer::_extract_xml_from_archive(mz_zip_archive&         archive,
                                                  const std::string&      path,
                                                  XML_StartElementHandler start_handler,
                                                  XML_EndElementHandler   end_handler)
{
    return _extract_from_archive(archive, path,
                                 [this, start_handler, end_handler](mz_zip_archive& archive, const mz_zip_archive_file_stat& stat) {
                                                  ...
                                                  XML_EndElementHandler           end_handler)
{
    ...
    if (!XML_ParseBuffer(m_xml_parser, (int) stat.m_uncomp_size, 1)) {
        char error_buf[1024];
        ::snprintf(error_buf, 1024, "Error (%s) while parsing xml file at line %d", XML_ErrorString(XML_GetErrorCode(m_xml_parser)),
...
bool _BBS_3MF_Importer::_extract_model_from_archive(mz_zip_archive& archive, const mz_zip_archive_file_stat& stat)
{
    ...
    XML_SetUserData(m_xml_parser, (void*) this);
    XML_SetElementHandler(m_xml_parser, _BBS_3MF_Importer::_handle_start_model_xml_element,
                          _BBS_3MF_Importer::_handle_end_model_xml_element);
    XML_SetCharacterDataHandler(m_xml_parser, _BBS_3MF_Importer::_handle_xml_characters);
    ...
    struct CallbackData
    {
        ...
        CallbackData(XML_Parser& parser, _BBS_3MF_Importer& importer, const mz_zip_archive_file_stat& stat)
    ...
    try {
        mz_file_write_func callback = [](void* pOpaque, mz_uint64 file_ofs, const void* pBuf, size_t n) -> size_t {
            ...
            if (!XML_Parse(data->parser, (const char*) pBuf, (int) n, (file_ofs + n == data->stat.m_uncomp_size) ? 1 : 0) ||
                data->importer.parse_error()) {
                char error_buf[1024];
                ::snprintf(error_buf, 1024, "Error (%s) while parsing '%s' at line %d", data->importer.parse_error_message(),
...
void _BBS_3MF_Importer::_extract_cut_information_from_archive(mz_zip_archive&                 archive,
                                                              const mz_zip_archive_file_stat& stat,
                                                              ConfigSubstitutionContext&      config_substitutions)
{
    if (stat.m_uncomp_size > 0) {
...
void _BBS_3MF_Importer::_extract_print_config_from_archive(mz_zip_archive&                 archive,
                                                           const mz_zip_archive_file_stat& stat,
                                                           DynamicPrintConfig&             config,
                                                           ConfigSubstitutionContext&      config_substitutions,
                                                           const std::string&              archive_filename)
...
void _BBS_3MF_Importer::_extract_project_config_from_archive(mz_zip_archive&                 archive,
                                                             const mz_zip_archive_file_stat& stat,
                                                             DynamicPrintConfig&             config,
                                                             ConfigSubstitutionContext&      config_substitutions,
                                                             Model&                          model)
...
// BBS: extract project embedded presets
void _BBS_3MF_Importer::_extract_project_embedded_presets_from_archive(mz_zip_archive&                 archive,
                                                                       const mz_zip_archive_file_stat& stat,
                                                                       std::vector<Preset*>&           project_presets,
                                                                       Model&                          model,
                                                                       Preset::Type                    type,
                                                                       bool                            use_json)
{
    if (stat.m_uncomp_size > 0) {
        ...
        std::map<std::string, std::string> key_values;
        std::string                        reason;
        ConfigSubstitutions                config_substitutions = use_json ? config.load_from_json(dest_file, Enable, key_values, reason) :
                                                                             config.load_from_ini(dest_file, Enable);
        if (!reason.empty()) {
        ...
        if (type == Preset::TYPE_PRINT) {
            ...
            BOOST_LOG_TRIVIAL(error) << __FUNCTION__
                                     << boost::format(", invalid type  %1% from file %2%\n") % Preset::get_type_string(type) % dest_file;
            // skip this file
            return;
        }
        ...
        BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                << boost::format(", create one project embedded preset: %1% from %2%, type %3%\n") % preset_name %
                                       dest_file % Preset::get_type_string(type);
    }
}
...
void _BBS_3MF_Importer::_extract_layer_config_ranges_from_archive(mz_zip_archive&                 archive,
                                                                  const mz_zip_archive_file_stat& stat,
                                                                  ConfigSubstitutionContext&      config_substitutions)
{
    if (stat.m_uncomp_size > 0) {
...
void _BBS_3MF_Importer::_extract_embossed_svg_shape_file(const std::string&              filename,
                                                         mz_zip_archive&                 archive,
                                                         const mz_zip_archive_file_stat& stat)
{
        ...
        auto extract_code = [this](int plate_id, pt::ptree code_tree) {
            for (const auto& code : code_tree) {
                if (code.first == "mode") {
                    ...
                    m_model->plates_custom_gcodes[plate_id - 1].mode = mode == CustomGCode::SingleExtruderMode ?
                ...
                if (code.first == "layer") {
                    ...
                    if (attr_tree.find("type") == attr_tree.not_found()) {
                        ...
                        type  = gcode == "M600"        ? CustomGCode::ColorChange :
                                ...
                                gcode == "tool_change" ? CustomGCode::ToolChange :
                                                         CustomGCode::Custom;
                        extra = type == CustomGCode::PausePrint ? color : type == CustomGCode::Custom ? gcode : "";
                    } else {
...
void _BBS_3MF_Importer::_handle_xml_characters(const XML_Char* s, int len) { m_curr_characters.append(s, len); }
        ...
        if (m_is_bbl_3mf && boost::ends_with(m_curr_object->uuid, OBJECT_UUID_SUFFIX) && m_load_restore) {
            ...
            if (!m_curr_object->components.empty()) {
                ...
                for (int index = 0; index < m_curr_object->components.size(); index++) {
                    ...
                    if (current_object != m_current_objects.end() && (new_id.first.empty() || new_id.first == current_object->first.first)) {
...
bool _BBS_3MF_Importer::_handle_start_vertex(const char** attributes, unsigned int num_attributes)
{
    ...
    if (m_curr_object)
        m_curr_object->geometry.vertices.emplace_back(m_unit_factor * bbs_get_attribute_value_float(attributes, num_attributes, X_ATTR),
...
bool _BBS_3MF_Importer::_handle_start_triangle(const char** attributes, unsigned int num_attributes)
{
    ...
    // missing values are set equal to ZERO
    if (m_curr_object) {
        m_curr_object->geometry.triangles.emplace_back(bbs_get_attribute_value_int(attributes, num_attributes, V1_ATTR),
        ...
        m_curr_object->geometry.mmu_segmentation.push_back(
            bbs_get_attribute_value_string(attributes, num_attributes, MMU_SEGMENTATION_ATTR));
        // BBS
...
bool _BBS_3MF_Importer::_handle_start_component(const char** attributes, unsigned int num_attributes)
{
    ...
    Transform3d transform = bbs_get_transform_from_3mf_specs_string(
        bbs_get_attribute_value_string(attributes, num_attributes, TRANSFORM_ATTR));
...
bool _BBS_3MF_Importer::_handle_end_metadata()
{
    if ((m_curr_metadata_name == BBS_3MF_VERSION) || (m_curr_metadata_name == BBS_3MF_VERSION1)) {
        ...
        /*if (m_check_version && (m_version > VERSION_BBS_3MF_COMPATIBLE)) {
            // std::string msg = _(L("The selected 3mf file has been saved with a newer version of " + std::string(SLIC3R_APP_NAME) + " and
        is not compatible."));
            // throw version_error(msg.c_str());
            const std::string msg = (boost::format(_(L("The selected 3mf file has been saved with a newer version of %1% and is not
        compatible."))) % std::string(SLIC3R_APP_NAME)).str(); throw version_error(msg);
        }*/
    ...
    if (!m_curr_metadata_name.empty()) {
        BOOST_LOG_TRIVIAL(info) << "load_3mf found metadata key = " << m_curr_metadata_name
                                << ", value = " << xml_unescape(m_curr_characters);
...
bool _BBS_3MF_Importer::_create_object_instance(
    std::string const& path, int object_id, const Transform3d& transform, const bool printable, unsigned int recur_counter)
{
...
bool _BBS_3MF_Importer::_handle_start_config_metadata(const char** attributes, unsigned int num_attributes)
{
    ...
    if ((m_curr_plater == nullptr) && !m_parsing_slice_info) {
        ...
        // }
            ...
            if (object_item == m_objects.end()) {
                BOOST_LOG_TRIVIAL(warning) << __FUNCTION__ << ":" << __LINE__
                    << boost::format(", can not find object for plate's item, ID <%1%, %2%>, skip this object")%index_iter->second
            %index_iter->first; return true;
            }
...
bool _BBS_3MF_Importer::_handle_start_assemble(const char** attributes, unsigned int num_attributes) { return true; }
...
bool _BBS_3MF_Importer::_handle_end_assemble_item() { return true; }
...
bool _BBS_3MF_Importer::_handle_end_text_info_item() { return true; }
...
void _BBS_3MF_Importer::_generate_current_object_list(std::vector<Component>& sub_objects,
                                                      Id                      object_id,
                                                      IdToCurrentObjectMap&   current_objects)
{
...
bool _BBS_3MF_Importer::_generate_volumes_new(ModelObject&                              object,
                                              const std::vector<Component>&             sub_objects,
                                              const ObjectMetadata::VolumeMetadataList& volumes,
                                              ConfigSubstitutionContext&                config_substitutions)
{
    ...
    for (unsigned int index = 0; index < sub_objects.size(); index++) {
        ...
        BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                << boost::format(": line %1%, subobject_id %2%, shared_mesh_id %3%") % __LINE__ % sub_object->id %
                                       shared_mesh_id;
        if (shared_mesh_id != -1) {
            ...
            if (iter != m_shared_meshes.end()) {
                ...
                BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                        << boost::format(": line %1%, found shared mesh, id %2%, mesh %3%") % __LINE__ % shared_mesh_id %
                                               shared_volume;
            }
        } else {
            ...
            if (iter != m_shared_meshes.end()) {
                shared_volume = iter->second;
                BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                        << boost::format(": line %1%, already loaded copy-share mesh before, id %2%, mesh %3%") % __LINE__ %
                                               sub_object->id % shared_volume;
            }
        }
        ...
        if (!shared_volume) {
            ...
            BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                    << boost::format(": line %1%, create volume using shared_mesh %2%") % __LINE__ % shared_volume;
        }
...
/*
bool _BBS_3MF_Importer::_generate_volumes(ModelObject& object, const Geometry& geometry, const ObjectMetadata::VolumeMetadataList& volumes,
ConfigSubstitutionContext& config_substitutions)
{
    if (!object.volumes.empty()) {
    ...
    for (const ObjectMetadata::VolumeMetadata& volume_data : volumes) {
        if (geo_tri_count <= volume_data.first_triangle_id || geo_tri_count <= volume_data.last_triangle_id || volume_data.last_triangle_id
< volume_data.first_triangle_id) { add_error("Found invalid triangle id"); return false;
        }
        ...
        // splits volume out of imported geometry
        indexed_triangle_set its;
        its.indices.assign(geometry.triangles.begin() + volume_data.first_triangle_id, geometry.triangles.begin() +
volume_data.last_triangle_id + 1); const size_t triangles_count = its.indices.size(); if (triangles_count == 0) { add_error("An empty
triangle mesh found"); return false;
        }
...
bool _BBS_3MF_Importer::ObjectImporter::_handle_object_start_vertex(const char** attributes, unsigned int num_attributes)
{
    ...
    // missing values are set equal to ZERO
    if (current_object)
        current_object->geometry.vertices
            .emplace_back(object_unit_factor * bbs_get_attribute_value_float(attributes, num_attributes, X_ATTR),
...
bool _BBS_3MF_Importer::ObjectImporter::_handle_object_start_triangle(const char** attributes, unsigned int num_attributes)
{
    ...
    // missing values are set equal to ZERO
    if (current_object) {
        current_object->geometry.triangles.emplace_back(bbs_get_attribute_value_int(attributes, num_attributes, V1_ATTR),
                                                        bbs_get_attribute_value_int(attributes, num_attributes, V2_ATTR),
                                                        bbs_get_attribute_value_int(attributes, num_attributes, V3_ATTR));
        ...
        current_object->geometry.mmu_segmentation.push_back(
            bbs_get_attribute_value_string(attributes, num_attributes, MMU_SEGMENTATION_ATTR));
        // BBS
...
void _BBS_3MF_Importer::ObjectImporter::_handle_object_xml_characters(const XML_Char* s, int len) { obj_curr_characters.append(s, len); }

void XMLCALL _BBS_3MF_Importer::ObjectImporter::_handle_object_start_model_xml_element(void*        userData,
                                                                                       const char*  name,
...
bool _BBS_3MF_Importer::ObjectImporter::_extract_object_from_archive(mz_zip_archive& archive, const mz_zip_archive_file_stat& stat)
{
    ...
    XML_SetElementHandler(object_xml_parser, _BBS_3MF_Importer::ObjectImporter::_handle_object_start_model_xml_element,
                          _BBS_3MF_Importer::ObjectImporter::_handle_object_end_model_xml_element);
    ...
    try {
        mz_file_write_func callback = [](void* pOpaque, mz_uint64 file_ofs, const void* pBuf, size_t n) -> size_t {
            CallbackData* data = (CallbackData*) pOpaque;
            if (!XML_Parse(data->parser, (const char*) pBuf, (int) n, (file_ofs + n == data->stat.m_uncomp_size) ? 1 : 0) ||
                data->importer.object_parse_error()) {
                char error_buf[1024];
                ::snprintf(error_buf, 1024, "Error (%s) while parsing '%s' at line %d", data->importer.object_parse_error_message(),
                           data->stat.m_filename, (int) XML_GetCurrentLineNumber(data->parser));
                throw Slic3r::FileIOError(error_buf);
            }
            return n;
...
class _BBS_3MF_Exporter : public _BBS_3MF_Base
{
    struct BuildItem
    {
        ...
        BuildItem(std::string const& path, unsigned int id, const Transform3d& transform, const bool printable)
            : path(path), id(id), transform(transform), printable(printable)
        {}
    };
...
public:
    ...
    // add backup logic
    // bool save_model_to_file(const std::string& filename, Model& model, PlateDataPtrs& plate_data_list, std::vector<Preset*>&
    // project_presets, const DynamicPrintConfig* config, bool fullpath_sources, const std::vector<ThumbnailData*>& thumbnail_data, bool
    // zip64, bool skip_static, Export3mfProgressFn proFn = nullptr, bool silence = false);
...
private:
    ...
    bool _add_thumbnail_file_to_archive(mz_zip_archive&      archive,
                                        const ThumbnailData& thumbnail_data,
                                        const char*          local_path,
                                        int                  index,
                                        bool                 generate_small_thumbnail = false);
    ...
    bool _add_model_file_to_archive(const std::string&     filename,
                                    ...
                                    BBLProject*            project = nullptr) const;
    ...
    bool _add_model_config_file_to_archive(mz_zip_archive&              archive,
                                           const Model&                 model,
                                           PlateDataPtrs&               plate_data_list,
                                           const ObjectToObjectDataMap& objects_data,
                                           int                          export_plate_idx = -1,
                                           bool                         save_gcode       = true,
                                           bool                         use_loaded_id    = false);
    bool _add_cut_information_file_to_archive(mz_zip_archive& archive, Model& model);
    bool _add_slice_info_config_file_to_archive(mz_zip_archive&              archive,
                                                const Model&                 model,
                                                PlateDataPtrs&               plate_data_list,
                                                const ObjectToObjectDataMap& objects_data,
                                                const DynamicPrintConfig&    config);
    bool _add_gcode_file_to_archive(mz_zip_archive&     archive,
                                    const Model&        model,
                                    PlateDataPtrs&      plate_data_list,
                                    Export3mfProgressFn proFn = nullptr);
...
bool _BBS_3MF_Exporter::save_model_to_file(StoreParams& store_params)
{
    ...
    bool result = _save_model_to_file(filename + ".tmp", *store_params.model, store_params.plate_data_list, store_params.project_presets,
                                      store_params.config, store_params.thumbnail_data, store_params.no_light_thumbnail_data,
                                      store_params.top_thumbnail_data, store_params.pick_thumbnail_data, store_params.proFn,
                                      store_params.calibration_thumbnail_data, store_params.id_bboxes, store_params.project,
                                            ...
                                            int                                export_plate_idx)
{
    ...
    // BBS progress point
    BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ":" << __LINE__
                            << boost::format(",before open zip writer, m_skip_static %1%, m_save_gcode %2%, m_use_loaded_id %3%") %
                                   m_skip_static % m_save_gcode % m_use_loaded_id;
    if (proFn) {
    ...
    // BBS progress point
    BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ":" << __LINE__
                            << boost::format(",before add thumbnails, count %1%") % thumbnail_data.size();
    BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ":" << __LINE__
                            << boost::format(",top&&pick thumbnails, count %1%") % top_thumbnail_data.size();
    ...
    if (!m_skip_static) {
        ...
        if ((thumbnail_data.size() > 0) && (thumbnail_data.size() > plate_data_list.size())) {
            BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << ":" << __LINE__
                                     << boost::format(", thumbnail_data size %1% > plate count %2%") % thumbnail_data.size() %
                                            plate_data_list.size();
            return false;
        }
        if ((no_light_thumbnail_data.size() > 0) && (no_light_thumbnail_data.size() > plate_data_list.size())) {
            BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << ":" << __LINE__
                                     << boost::format(", no_light_thumbnail_data size %1% > plate count %2%") %
        ...
        if ((top_thumbnail_data.size() > 0) && (top_thumbnail_data.size() > plate_data_list.size())) {
            BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << ":" << __LINE__
                                     << boost::format(", top_thumbnail_data size %1% > plate count %2%") % top_thumbnail_data.size() %
                                            plate_data_list.size();
            return false;
        }
        if ((pick_thumbnail_data.size() > 0) && (pick_thumbnail_data.size() > plate_data_list.size())) {
            BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << ":" << __LINE__
                                     << boost::format(", pick_thumbnail_data size %1% > plate count %2%") % pick_thumbnail_data.size() %
                                            plate_data_list.size();
            return false;
        }
        if (top_thumbnail_data.size() != pick_thumbnail_data.size()) {
            BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << ":" << __LINE__
                                     << boost::format(", top_thumbnail_data size %1% != pick_thumbnail_data size %2%") %
        ...
        for (unsigned int index = 0; index < thumbnail_data.size(); index++) {
            if (thumbnail_data[index]->is_valid()) {
                ...
                BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ":" << __LINE__
                                        << boost::format(",add thumbnail %1%'s data into 3mf") % (index + 1);
        ...
        for (unsigned int index = 0; index < no_light_thumbnail_data.size(); index++) {
            if (no_light_thumbnail_data[index]->is_valid()) {
                ...
                BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ":" << __LINE__
                                        << boost::format(",add no light thumbnail %1%'s data into 3mf") % (index + 1);
                thumbnail_status[index] = true;
            }
        }
        ...
        for (unsigned int index = 0; index < top_thumbnail_data.size(); index++) {
            if (top_thumbnail_data[index]->is_valid()) {
                BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ":" << __LINE__
                                        << boost::format(",add top thumbnail %1%'s data into 3mf") % (index + 1);
            ...
            if (pick_thumbnail_data[index]->is_valid()) {
                BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ":" << __LINE__
                                        << boost::format(",add pick thumbnail %1%'s data into 3mf") % (index + 1);
        ...
        for (int i = 0; i < plate_data_list.size(); i++) {
            ...
            if (!thumbnail_status[i] && !plate_data->thumbnail_file.empty() && (boost::filesystem::exists(plate_data->thumbnail_file))) {
                ...
                BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ":" << __LINE__
                                        << boost::format(", add thumbnail %1% from file %2%") % (i + 1) % plate_data->thumbnail_file;

                if (!_add_file_to_archive(archive, dst_in_3mf, plate_data->thumbnail_file)) {
                    BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << ":" << __LINE__
                                             << boost::format(", add thumbnail %1% from file %2% failed\n") % (i + 1) %
                                                    plate_data->thumbnail_file;
                    return false;
                }
            }

            if (!no_light_thumbnail_status[i] && !plate_data->no_light_thumbnail_file.empty() &&
                ...
                BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ":" << __LINE__
                                        << boost::format(", add no light thumbnail %1% from file %2%") % (i + 1) %
                                               plate_data->no_light_thumbnail_file;

                if (!_add_file_to_archive(archive, dst_in_3mf, plate_data->no_light_thumbnail_file)) {
                    BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << ":" << __LINE__
                                             << boost::format(", add no light thumbnail %1% from file %2% failed\n") % (i + 1) %
                                                    plate_data->no_light_thumbnail_file;
                    return false;
                }
            }
            ...
            if (!top_thumbnail_status[i] && !plate_data->top_file.empty() && (boost::filesystem::exists(plate_data->top_file))) {
                ...
                BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ":" << __LINE__
                                        << boost::format(", add top thumbnail %1% from file %2%") % (i + 1) % plate_data->top_file;
                if (!_add_file_to_archive(archive, dst_in_3mf, plate_data->top_file)) {
                    BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << ":" << __LINE__
                                             << boost::format(", add top thumbnail %1% failed") % (i + 1);
                    return false;
                }
            ...
            if (!pick_thumbnail_status[i] && !plate_data->pick_file.empty() && (boost::filesystem::exists(plate_data->pick_file))) {
                ...
                BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ":" << __LINE__
                                        << boost::format(", add pick thumbnail %1% from file %2%") % (i + 1) % plate_data->pick_file;
                if (!_add_file_to_archive(archive, dst_in_3mf, plate_data->pick_file)) {
                    BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << ":" << __LINE__
                                             << boost::format(", add pick thumbnail %1% failed") % (i + 1);
                    return false;
                }
    ...
    BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ":" << __LINE__
                            << boost::format(",before add calibration thumbnails, count %1%\n") % calibration_data.size();
    ...
    BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ":" << __LINE__
                            << boost::format(",before add calibration boundingbox, count %1%\n") % id_bboxes.size();
    ...
    // if (!m_skip_model)
    {
        ...
        if (project_presets.size() > 0) {
            ...
            BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ":" << __LINE__
                                    << boost::format(", finished add project embedded settings, size %1%\n") % project_presets.size();
            if (proFn) {
    ...
    if (!m_skip_static && m_save_gcode) {
        for (int i = 0; i < plate_data_list.size(); i++) {
            ...
            if (!plate_data->gcode_file.empty() && plate_data->is_sliced_valid && boost::filesystem::exists(plate_data->gcode_file)) {
                ...
                if (!mz_zip_writer_add_mem(&archive, target_file.c_str(), (const void*) plate_data->gcode_file_md5.c_str(),
                                           plate_data->gcode_file_md5.length(), MZ_DEFAULT_COMPRESSION)) {
                    BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << ":" << __LINE__
    ...
    // As there is just a single Indexed Triangle Set data stored per ModelObject, offsets of volumes into their respective Indexed Triangle
    // Set data is stored here as well.
...
bool _BBS_3MF_Exporter::_add_file_to_archive(mz_zip_archive& archive, const std::string& path_in_zip, const std::string& src_file_path)
{
    ...
    bool nocomp = std::find_if(nocomp_exts, end, [&path_in_zip](auto& ext) { return boost::algorithm::ends_with(path_in_zip, ext); }) !=
                  end;
#if WRITE_ZIP_LANGUAGE_ENCODING
    bool result = mz_zip_writer_add_file(&archive, path_in_zip.c_str(), encode_path(src_file_path.c_str()).c_str(), NULL, 0,
    ...
    bool        result      = mz_zip_writer_add_file_ex(&archive, native_path.c_str(), encode_path(src_file_path.c_str()).c_str(), NULL, 0,
                                            nocomp ? MZ_ZIP_FLAG_ASCII_FILENAME : MZ_DEFAULT_COMPRESSION, extra.c_str(), extra.length(),
                                                        extra.c_str(), extra.length());
#endif
    if (!result) {
        add_error("Unable to add file " + src_file_path + " to archive");
        BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << ":" << __LINE__
                                 << boost::format(", Unable to add file %1% to archive %2%\n") % src_file_path % path_in_zip;
    } else {
        BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ":" << __LINE__
                                << boost::format(", add file %1% to archive %2%\n") % src_file_path % path_in_zip;
    }
    return result;
}
...
bool _BBS_3MF_Exporter::_add_thumbnail_file_to_archive(
    mz_zip_archive& archive, const ThumbnailData& thumbnail_data, const char* local_path, int index, bool generate_small_thumbnail)
{
    bool res = false;

    size_t png_size = 0;
    void*  png_data = tdefl_write_image_to_png_file_in_memory_ex((const void*) thumbnail_data.pixels.data(), thumbnail_data.width,
                                                                 thumbnail_data.height, 4, &png_size, MZ_DEFAULT_COMPRESSION, 1);
    if (png_data != nullptr) {
    ...
    if (generate_small_thumbnail && thumbnail_data.is_valid()) {
        ...
        for (int i = 0; i < clampped_height; i += sh) {
            for (int j = 0; j < clampped_width; j += sw) {
                ...
                // memcpy((void*)&small_pixels[4*(i / sw * PLATE_THUMBNAIL_SMALL_WIDTH + j / sh)], thumbnail_data.pixels.data() + 4*(i *
                // thumbnail_data.width + j), 4);
            }
        }
        size_t small_png_size = 0;
        void*  small_png_data = tdefl_write_image_to_png_file_in_memory_ex((const void*) small_pixels.data(), PLATE_THUMBNAIL_SMALL_WIDTH,
                                                                           PLATE_THUMBNAIL_SMALL_HEIGHT, 4, &small_png_size,
        ...
        if (!res) {
            ...
            BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << ":" << __LINE__
                                     << boost::format(", Unable to add small thumbnail file to archive\n");
        }
    }
...
bool _BBS_3MF_Exporter::_add_calibration_file_to_archive(mz_zip_archive& archive, const ThumbnailData& thumbnail_data, int index)
{
    ...
    /*size_t png_size = 0;
    void* png_data = tdefl_write_image_to_png_file_in_memory_ex((const void*)thumbnail_data.pixels.data(), thumbnail_data.width,
    thumbnail_data.height, 4, &png_size, MZ_DEFAULT_COMPRESSION, 1); if (png_data != nullptr) { std::string thumbnail_name =
    (boost::format(PATTERN_FILE_FORMAT) % (index + 1)).str(); res = mz_zip_writer_add_mem(&archive, thumbnail_name.c_str(), (const
    void*)png_data, png_size, MZ_NO_COMPRESSION); mz_free(png_data);
    }
...
bool _BBS_3MF_Exporter::_add_relationships_file_to_archive(mz_zip_archive&                 archive,
                                                           ...
                                                           std::vector<std::string> const& targets,
                                                           std::vector<std::string> const& types,
                                                           PackingTemporaryData            data,
                                                           int                             export_plate_idx) const
{
    std::stringstream stream;
    stream << "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n";
    stream << "<Relationships xmlns=\"http://schemas.openxmlformats.org/package/2006/relationships\">\n";
    if (from.empty()) {
        stream << " <Relationship Target=\"/" << MODEL_FILE
    ...
    if (!mz_zip_writer_add_mem(&archive, from.empty() ? RELATIONSHIPS_FILE.c_str() : from.c_str(), (const void*) out.data(), out.length(),
...
bool _BBS_3MF_Exporter::_add_model_file_to_archive(const std::string&     filename,
                                                   ...
                                                   ObjectToObjectDataMap& objects_data,
                                                   Export3mfProgressFn    proFn,
                                                   BBLProject*            project) const
{
    bool sub_model    = !objects_data.empty();
    bool write_object = sub_model || !m_split_model;

    BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ":" << __LINE__
                            << boost::format(", filename %1%, m_split_model %2%,  sub_model %3%") % filename % m_split_model % sub_model;
...
#else
                                       nullptr, nullptr, 0, MZ_DEFAULT_COMPRESSION, extra.c_str(), extra.length(), extra.c_str(),
                                       extra.length())) {
#endif
...

    {
        ...
        if (!model.md_name.empty()) {
            for (unsigned int i = 0; i < model.md_name.size(); i++) {
                BOOST_LOG_TRIVIAL(info) << boost::format("saved metadata_name %1%, metadata_value %2%") % model.md_name[i] %
                                               model.md_value[i];
                ...
                if (m_share_mesh) {
                    ...
                    if (iter != m_shared_meshes.end()) {
                        ...
                        if ((shared_volume->supported_facets.equals(volume->supported_facets)) &&
                            ...
                            const_cast<_BBS_3MF_Exporter*>(this)->m_volume_paths.insert(
                                {volume, {data->sub_path, data->volumes_objectID.find(iter->second.second)->second}});
    ...
    // write model rels
    _add_relationships_file_to_archive(archive, MODEL_RELS_FILE, object_paths,
                                       {"http://schemas.microsoft.com/3dmanufacturing/2013/01/3dmodel"});

    if (!m_from_backup_save) {
        boost::mutex mutex;
        tbb::parallel_for(tbb::blocked_range<size_t>(0, objects_data.size(), 1),
                          [this, &mutex, &model, objects = model.objects, &objects_data, &object_paths, main = &archive,
                           project](const tbb::blocked_range<size_t>& range) {
...
void _BBS_3MF_Exporter::_add_object_components_to_stream(std::stringstream& stream, ObjectData const& object_data) const
{
    ...
    for (unsigned int index = 0; index < object.volumes.size(); index++) {
        ...
        if (m_production_ext)
            stream << "\" " << PUUID_ATTR << "=\"" << hex_wrap<boost::uint32_t>{(boost::uint32_t) (index + (object_data.backup_id << 16))}
                   << COMPONENT_UUID_SUFFIX;
...
bool _BBS_3MF_Exporter::_add_mesh_to_object_stream(std::function<bool(std::string&, bool)> const& flush, ObjectData const& object_data) const
{
    ...
    for (unsigned int index = 0; index < object.volumes.size(); index++) {
        ...
        // This won't break anything because when loading the file Orca (and Bambu) simply does not care about the actual object type at all
        // (as long as it's one of "model" & "other"); But PrusaSlicer requires the type to be "model".
        ...
        for (int i = 0; i < int(its.indices.size()); ++i) {
            {
                ...
                boost::spirit::karma::generate(ptr,
                                               boost::spirit::lit("     <")
                                                   << TRIANGLE_TAG << " v1=\"" << boost::spirit::int_ << "\" v2=\"" << boost::spirit::int_
                                                   << "\" v3=\"" << boost::spirit::int_ << "\"",
                                               idx[is_left_handed ? 2 : 0], idx[1], idx[is_left_handed ? 0 : 2]);
                *ptr = '\0';
                output_buffer += buf;
            }
...
bool _BBS_3MF_Exporter::_add_layer_height_profile_file_to_archive(mz_zip_archive& archive, Model& model)
{
    ...
    if (!out.empty()) {
        if (!mz_zip_writer_add_mem(&archive, BBS_LAYER_HEIGHTS_PROFILE_FILE.c_str(), (const void*) out.data(), out.length(),
                                   MZ_DEFAULT_COMPRESSION)) {
            add_error("Unable to add layer heights profile file to archive");
            BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << ":" << __LINE__
                                     << boost::format("Unable to add layer heights profile file to archive\n");
            return false;
        }
    }
...
bool _BBS_3MF_Exporter::_add_layer_config_ranges_file_to_archive(mz_zip_archive& archive, Model& model)
{
    ...
    if (!out.empty()) {
        if (!mz_zip_writer_add_mem(&archive, LAYER_CONFIG_RANGES_FILE.c_str(), (const void*) out.data(), out.length(),
                                   MZ_DEFAULT_COMPRESSION)) {
            add_error("Unable to add layer heights profile file to archive");
            BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << ":" << __LINE__
                                     << boost::format("Unable to add layer heights profile file to archive\n");
    ...
    for (const ModelObject* object : model.objects) {
        ...
        if (!brim_points.empty()) {
            ...
            for (size_t i = 0; i < brim_points.size(); ++i) {
                sprintf(buffer, (i == 0 ? "%f %f %f %f" : " %f %f %f %f"), brim_points[i].pos(0), brim_points[i].pos(1),
                        brim_points[i].pos(2), brim_points[i].head_front_radius);
                out += buffer;
            }
            out += "\n";
        }
    ...
    for (const ModelObject* object : model.objects) {
        ...
        if (!sla_support_points.empty()) {
            ...
            for (size_t i = 0; i < sla_support_points.size(); ++i) {
                sprintf(buffer, (i==0 ? "%f %f %f %f %f" : " %f %f %f %f %f"),  sla_support_points[i].pos(0), sla_support_points[i].pos(1),
sla_support_points[i].pos(2), sla_support_points[i].head_front_radius, (float)sla_support_points[i].is_new_island); out += buffer;
            }
            out += "\n";
        }
    }
    ...
    if (!out.empty()) {
        ...
        if (!mz_zip_writer_add_mem(&archive, SLA_SUPPORT_POINTS_FILE.c_str(), (const void*)out.data(), out.length(),
MZ_DEFAULT_COMPRESSION)) { add_error("Unable to add sla support points file to archive"); BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << ":" <<
__LINE__ << boost::format("Unable to add sla support points file to archive\n"); return false;
        }
    }
    return true;
}
...
bool _BBS_3MF_Exporter::_add_sla_drain_holes_file_to_archive(mz_zip_archive& archive, Model& model)
{
    ...
    if (!out.empty()) {
        ...
        if (!mz_zip_writer_add_mem(&archive, SLA_DRAIN_HOLES_FILE.c_str(), static_cast<const void*>(out.data()), out.length(),
mz_uint(MZ_DEFAULT_COMPRESSION))) { add_error("Unable to add sla support points file to archive"); BOOST_LOG_TRIVIAL(error) << __FUNCTION__
<< ":" << __LINE__ << boost::format("Unable to add sla support points file to archive\n"); return false;
        }
    }
    return true;
...
bool _BBS_3MF_Exporter::_add_print_config_file_to_archive(mz_zip_archive& archive, const DynamicPrintConfig& config)
{
    ...
    if (!out.empty()) {
        if (!mz_zip_writer_add_mem(&archive, BBS_PRINT_CONFIG_FILE.c_str(), (const void*) out.data(), out.length(),
                                   MZ_DEFAULT_COMPRESSION)) {
...
bool _BBS_3MF_Exporter::_add_model_config_file_to_archive(mz_zip_archive&              archive,
                                                          const Model&                 model,
                                                          PlateDataPtrs&               plate_data_list,
                                                          const ObjectToObjectDataMap& objects_data,
                                                          int                          export_plate_idx,
                                                          ...
                                                          bool                         use_loaded_id)
{
    ...
    if (!m_skip_model)
        for (const ObjectToObjectDataMap::value_type& obj_metadata : objects_data) {
            ...
            if (obj != nullptr) {
                ...
                // stream << "  <"  << OBJECT_TAG << " " << ID_ATTR << "=\"" << obj_metadata.first << "\" " << INSTANCESCOUNT_ATTR << "=\""
                // << obj->instances.size() << "\">\n";
                ...
                if (!obj->module_name.empty())
                    stream << "    <" << METADATA_TAG << " " << KEY_ATTR << "=\"module\" " << VALUE_ATTR << "=\""
                           << xml_escape(obj->module_name) << "\"/>\n";

                // stores object's config data
                for (const std::string& key : obj->config.keys()) {
                    stream << "    <" << METADATA_TAG << " " << KEY_ATTR << "=\"" << key << "\" " << VALUE_ATTR << "=\""
                           << obj->config.opt_serialize(key) << "\"/>\n";
                }
                ...
                for (const ModelVolume* volume : obj_metadata.second.object->volumes) {
                    if (volume != nullptr) {
                        ...
                        if (it != objectIDs.end()) {
                            ...
                            // stream << "    <" << PART_TAG << " " << ID_ATTR << "=\"" << it->second << "\" " << SUBTYPE_ATTR << "=\"" <<
                            // ModelVolume::type_to_string(volume->type()) << "\">\n";

                            // stores volume's name
                            if (!volume->name.empty())
                                stream << "      <" << METADATA_TAG << " " << KEY_ATTR << "=\"" << NAME_KEY << "\" " << VALUE_ATTR << "=\""
                            ...
                            /*if (volume->is_modifier())
                                stream << "      <" << METADATA_TAG << " " << TYPE_ATTR << "=\"" << PART_TYPE << "\" " << KEY_ATTR << "=\""
                            << MODIFIER_KEY << "\" " << VALUE_ATTR << "=\"1\"/>\n";
                            // stores volume's type (overrides the modifier field above)
                            stream << "      <" << METADATA_TAG << " " << TYPE_ATTR << "=\"" << PART_TYPE << "\" " << KEY_ATTR << "=\"" <<
                            PART_TYPE_KEY << "\" " << VALUE_ATTR << "=\"" << ModelVolume::type_to_string(volume->type()) << "\"/>\n";*/
                            ...
                            // stores volume's source data
                            {
                                std::string input_file = xml_escape(
                                    m_fullpath_sources ? volume->source.input_file :
    ...
    for (unsigned int i = 0; i < (unsigned int) plate_data_list.size(); ++i) {
        ...
        if (plate_data != nullptr) {
            stream << "  <" << PLATE_TAG << ">\n";
            // plate index
            stream << "    <" << METADATA_TAG << " " << KEY_ATTR << "=\"" << PLATERID_ATTR << "\" " << VALUE_ATTR << "=\""
                   << plate_data->plate_index + 1 << "\"/>\n";
            stream << "    <" << METADATA_TAG << " " << KEY_ATTR << "=\"" << PLATER_NAME_ATTR << "\" " << VALUE_ATTR << "=\""
                   << xml_escape(plate_data->plate_name.c_str()) << "\"/>\n";
            stream << "    <" << METADATA_TAG << " " << KEY_ATTR << "=\"" << LOCK_ATTR << "\" " << VALUE_ATTR << "=\"" << std::boolalpha
                   << plate_data->locked << "\"/>\n";
            ...
            if (bed_type_opt != nullptr && bed_type_names.size() > bed_type_opt->getInt())
                stream << "    <" << METADATA_TAG << " " << KEY_ATTR << "=\"" << BED_TYPE_ATTR << "\" " << VALUE_ATTR << "=\""
            ...
            if (print_sequence_opt != nullptr && print_sequence_names.size() > print_sequence_opt->getInt())
                stream << "    <" << METADATA_TAG << " " << KEY_ATTR << "=\"" << PRINT_SEQUENCE_ATTR << "\" " << VALUE_ATTR << "=\""
                       << print_sequence_names[print_sequence_opt->getInt()] << "\"/>\n";
            ...
            const ConfigOptionInt* sequence_nums_opt = dynamic_cast<const ConfigOptionInt*>(
                plate_data->config.option("other_layers_print_sequence_nums"));
            if (sequence_nums_opt != nullptr) {
                stream << "    <" << METADATA_TAG << " " << KEY_ATTR << "=\"" << OTHER_LAYERS_PRINT_SEQUENCE_NUMS_ATTR << "\" "
                       << VALUE_ATTR << "=\"" << sequence_nums_opt->getInt() << "\"/>\n";
            }

            ConfigOption* spiral_mode_opt = plate_data->config.option("spiral_mode");
            if (spiral_mode_opt)
                stream << "    <" << METADATA_TAG << " " << KEY_ATTR << "=\"" << SPIRAL_VASE_MODE << "\" " << VALUE_ATTR << "=\""
                       << spiral_mode_opt->getBool() << "\"/>\n";

            if (save_gcode)
                stream << "    <" << METADATA_TAG << " " << KEY_ATTR << "=\"" << GCODE_FILE_ATTR << "\" " << VALUE_ATTR << "=\""
                       << std::boolalpha << xml_escape(plate_data->gcode_file) << "\"/>\n";
            ...
            if (plate_data->plate_thumbnail.is_valid()) {
                ...
                stream << "    <" << METADATA_TAG << " " << KEY_ATTR << "=\"" << THUMBNAIL_FILE_ATTR << "\" " << VALUE_ATTR << "=\""
            ...
            if (!plate_data->no_light_thumbnail_file.empty()) {
                std::string no_light_thumbnail_file_in_3mf = (boost::format(NO_LIGHT_THUMBNAIL_FILE_FORMAT) % (plate_data->plate_index + 1))
                                                                 .str();
                stream << "    <" << METADATA_TAG << " " << KEY_ATTR << "=\"" << NO_LIGHT_THUMBNAIL_FILE_ATTR << "\" " << VALUE_ATTR
                       << "=\"" << std::boolalpha << no_light_thumbnail_file_in_3mf << "\"/>\n";
            }

            if (!plate_data->top_file.empty()) {
                std::string top_file_in_3mf = (boost::format(TOP_FILE_FORMAT) % (plate_data->plate_index + 1)).str();
                stream << "    <" << METADATA_TAG << " " << KEY_ATTR << "=\"" << TOP_FILE_ATTR << "\" " << VALUE_ATTR << "=\""
                       << std::boolalpha << top_file_in_3mf << "\"/>\n";
            }

            if (!plate_data->pick_file.empty()) {
                std::string pick_file_in_3mf = (boost::format(PICK_FILE_FORMAT) % (plate_data->plate_index + 1)).str();
                stream << "    <" << METADATA_TAG << " " << KEY_ATTR << "=\"" << PICK_FILE_ATTR << "\" " << VALUE_ATTR << "=\""
                       << std::boolalpha << pick_file_in_3mf << "\"/>\n";
            }
            ...
            /*if (!plate_data->pattern_file.empty()) {
                ...
                stream << "    <" << METADATA_TAG << " " << KEY_ATTR << "=\"" << PATTERN_FILE_ATTR << "\" " << VALUE_ATTR << "=\"" <<
            std::boolalpha << pattern_file_in_3mf << "\"/>\n";
            }*/
            if (!plate_data->pattern_bbox_file.empty()) {
                ...
                stream << "    <" << METADATA_TAG << " " << KEY_ATTR << "=\"" << PATTERN_BBOX_FILE_ATTR << "\" " << VALUE_ATTR << "=\""
                       << std::boolalpha << pattern_bbox_file_in_3mf << "\"/>\n";
            }
                ...
                for (unsigned int j = 0; j < instance_size; ++j) {
                    ...
                    if (obj && (inst_id >= obj->instances.size())) {
                        BOOST_LOG_TRIVIAL(warning)
                            << __FUNCTION__ << ":" << __LINE__ << boost::format("invalid instance id %1%\n") % inst_id;
                    } else if (obj) {
                    ...
                    stream << "      <" << METADATA_TAG << " " << KEY_ATTR << "=\"" << OBJECT_ID_ATTR << "\" " << VALUE_ATTR << "=\""
                           << obj_id << "\"/>\n";
                    stream << "      <" << METADATA_TAG << " " << KEY_ATTR << "=\"" << INSTANCEID_ATTR << "\" " << VALUE_ATTR << "=\""
                           << inst_id << "\"/>\n";
                    stream << "      <" << METADATA_TAG << " " << KEY_ATTR << "=\"" << IDENTIFYID_ATTR << "\" " << VALUE_ATTR << "=\""
                           << identify_id << "\"/>\n";
                    stream << "    </" << INSTANCE_TAG << ">\n";
                }
            }
    ...
    // write model rels
    if (save_gcode)
        _add_relationships_file_to_archive(archive, BBS_MODEL_CONFIG_RELS_FILE, gcode_paths,
                                           {"http://schemas.bambulab.com/package/2021/gcode"}, Slic3r::PackingTemporaryData(),
                                           export_plate_idx);
...
bool _BBS_3MF_Exporter::_add_slice_info_config_file_to_archive(mz_zip_archive&              archive,
                                                               const Model&                 model,
                                                               PlateDataPtrs&               plate_data_list,
                                                               const ObjectToObjectDataMap& objects_data,
                                                               const DynamicPrintConfig&    config)
{
    ...
    stream << "    <" << SLICE_HEADER_ITEM_TAG << " " << KEY_ATTR << "=\"" << "X-BBL-Client-Type" << "\" " << VALUE_ATTR << "=\""
           << "slicer" << "\"/>\n";
    stream << "    <" << SLICE_HEADER_ITEM_TAG << " " << KEY_ATTR << "=\"" << "X-BBL-Client-Version" << "\" " << VALUE_ATTR << "=\""
           << convert_to_full_version(SoftFever_VERSION) << "\"/>\n";
    stream << "  </" << SLICE_HEADER_TAG << ">\n";
    ...
    for (unsigned int i = 0; i < (unsigned int) plate_data_list.size(); ++i) {
        ...
        if (plate_data != nullptr && plate_data->is_sliced_valid) {
            stream << "  <" << PLATE_TAG << ">\n";
            // plate index
            stream << "    <" << METADATA_TAG << " " << KEY_ATTR << "=\"" << PLATE_IDX_ATTR << "\" " << VALUE_ATTR << "=\""
                   << plate_data->plate_index + 1 << "\"/>\n";
            ...
            stream << "    <" << METADATA_TAG << " " << KEY_ATTR << "=\"" << PRINTER_MODEL_ID_ATTR << "\" " << VALUE_ATTR << "=\""
                   << plate_data->printer_model_id << "\"/>\n";
            stream << "    <" << METADATA_TAG << " " << KEY_ATTR << "=\"" << NOZZLE_DIAMETERS_ATTR << "\" " << VALUE_ATTR << "=\""
                   << plate_data->nozzle_diameters << "\"/>\n";
            stream << "    <" << METADATA_TAG << " " << KEY_ATTR << "=\"" << TIMELAPSE_TYPE_ATTR << "\" " << VALUE_ATTR << "=\""
                   << timelapse_type << "\"/>\n";
            // stream << "    <" << METADATA_TAG << " " << KEY_ATTR << "=\"" << TIMELAPSE_ERROR_CODE_ATTR << "\" " << VALUE_ATTR << "=\"" <<
            // plate_data->timelapse_warning_code << "\"/>\n";
            stream << "    <" << METADATA_TAG << " " << KEY_ATTR << "=\"" << SLICE_PREDICTION_ATTR << "\" " << VALUE_ATTR << "=\""
                   << plate_data->get_gcode_prediction_str() << "\"/>\n";
            stream << "    <" << METADATA_TAG << " " << KEY_ATTR << "=\"" << SLICE_WEIGHT_ATTR << "\" " << VALUE_ATTR << "=\""
                   << plate_data->get_gcode_weight_str() << "\"/>\n";
            stream << "    <" << METADATA_TAG << " " << KEY_ATTR << "=\"" << OUTSIDE_ATTR << "\" " << VALUE_ATTR << "=\"" << std::boolalpha
                   << plate_data->toolpath_outside << "\"/>\n";
            stream << "    <" << METADATA_TAG << " " << KEY_ATTR << "=\"" << SUPPORT_USED_ATTR << "\" " << VALUE_ATTR << "=\""
                   << std::boolalpha << plate_data->is_support_used << "\"/>\n";
            stream << "    <" << METADATA_TAG << " " << KEY_ATTR << "=\"" << LABEL_OBJECT_ENABLED_ATTR << "\" " << VALUE_ATTR << "=\""
                   << std::boolalpha << plate_data->is_label_object_enabled << "\"/>\n";
            ...
            for (auto it = plate_data->objects_and_instances.begin(); it != plate_data->objects_and_instances.end(); it++) {
                ...
                stream << "    <" << OBJECT_TAG << " " << IDENTIFYID_ATTR << "=\"" << std::to_string(identify_id) << "\" " << NAME_ATTR
                       << "=\"" << xml_escape(obj->name) << "\" " << SKIPPED_ATTR << "=\"" << (skipped ? "true" : "false") << "\" />\n";
            }

            for (auto it = plate_data->slice_filaments_info.begin(); it != plate_data->slice_filaments_info.end(); it++) {
                stream << "    <" << FILAMENT_TAG << " " << FILAMENT_ID_TAG << "=\"" << std::to_string(it->id + 1) << "\" "
                       << FILAMENT_TRAY_INFO_ID_TAG << "=\"" << it->filament_id << "\" " << FILAMENT_TYPE_TAG << "=\"" << it->type << "\" "
                       << FILAMENT_COLOR_TAG << "=\"" << it->color << "\" " << FILAMENT_USED_M_TAG << "=\"" << it->used_m << "\" "
...
bool _BBS_3MF_Exporter::_add_gcode_file_to_archive(mz_zip_archive&     archive,
                                                   const Model&        model,
                                                   PlateDataPtrs&      plate_data_list,
                                                   Export3mfProgressFn proFn)
{
                          ...
                          for (int i = range.begin(); i < range.end(); ++i) {
                              ...
                              mz_zip_writer_init_heap(&archive, 0, 1024 * 1024);
                              {
                                  mz_zip_writer_add_staged_open(&archive, &context, gcode_in_3mf.c_str(),
                                                                m_zip64 ? (uint64_t(1) << 30) * 16 : (uint64_t(1) << 32) - 1, nullptr,
...
bool _BBS_3MF_Exporter::_add_custom_gcode_per_print_z_file_to_archive(mz_zip_archive&           archive,
                                                                      Model&                    model,
                                                                      const DynamicPrintConfig* config)
{
    // BBS: add plate tree related logic
    ...
    for (auto custom_gcodes : model.plates_custom_gcodes) {
        ...
        for (const CustomGCode::Item& code : custom_gcodes.second.gcodes) {
            ...
            std::string gcode = // code.type == CustomGCode::ColorChange ? config->opt_string("color_change_gcode")    :
                ...
                code.type == CustomGCode::ToolChange ? "tool_change" :
                                                       code.extra;
    ...
    if (!out.empty()) {
        if (!mz_zip_writer_add_mem(&archive, CUSTOM_GCODE_PER_PRINT_Z_FILE.c_str(), (const void*) out.data(), out.length(),
                                   MZ_DEFAULT_COMPRESSION)) {
            add_error("Unable to add custom Gcodes per print_z file to archive");
            BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << ":" << __LINE__
                                     << boost::format(", Unable to add custom Gcodes per print_z file to archive\n");
            return false;
        }
    }
...
public:
    ...
    void push_object_gaurd(ModelObject& object) { m_gaurd_objects.push_back(std::make_pair(&object, 0)); }
    ...
    void remove_object_mesh(ModelObject& object) { push_task({RemoveObject, object.id().id, object.get_model()->get_backup_path()}); }
    ...
    {
        BOOST_LOG_TRIVIAL(info) << "remove_backup " << model.get_backup_path() << ", " << removeAll;
    ...
    bool has_other_changes(bool backup) { return backup ? m_other_changes_backup : m_other_changes; }
...
private:
    ...
    struct Task
    {
        ...
        friend bool operator==(Task const& l, Task const& r) { return l.type == r.type && l.id == r.id; }
        std::string to_string() const
        {
            constexpr char const* type_names[] = {"None", "Backup", "AddObject", "RemoveObject", "RemoveBackup", "Exit"};
            std::ostringstream    os;
            os << "{ type:" << type_names[type] << ", id:" << id << ", path:" << path << ", object:" << (object ? object->id().id : 0)
               << ", extra:" << delay << "}";
            return os.str();
        }
    };
...
private:
    ...
    void process_ui_task(Task& t, bool canceled = false)
    {
        ...
        case AddObject: m_temp_model.delete_object(t.object); break;
        case RemoveBackup:
            if (t.removeAll) {
                try {
...
bool load_bbs_3mf(const char*                path,
                  DynamicPrintConfig*        config,
                  ConfigSubstitutionContext* config_substitutions,
                  Model*                     model,
                  PlateDataPtrs*             plate_data_list,
                  ...
                  int                        plate_id)
{
    ...
    CNumericLocalesSetter locales_setter;
    _BBS_3MF_Importer     importer;
    bool res = importer.load_model_from_file(path, *model, *plate_data_list, *project_presets, *config, *config_substitutions, strategy,
                                             *is_bbl_3mf, *file_version, proFn, project, plate_id);
    importer.log_errors();
...
bool load_gcode_3mf_from_stream(
    std::istream& data, DynamicPrintConfig* config, Model* model, PlateDataPtrs* plate_data_list, Semver* file_version)
{
    CNumericLocalesSetter locales_setter;
    _BBS_3MF_Importer     importer;
...
void backup_soon() { _BBS_Backup_Manager::get().backup_soon(); }

void remove_backup(Model& model, bool removeAll) { _BBS_Backup_Manager::get().remove_backup(model, removeAll); }

void set_backup_interval(long interval) { _BBS_Backup_Manager::get().set_interval(interval); }

void set_backup_callback(std::function<void(int)> callback) { _BBS_Backup_Manager::get().set_post_callback(callback); }

void run_backup_ui_tasks() { _BBS_Backup_Manager::get().run_ui_tasks(); }
...
void put_other_changes() { _BBS_Backup_Manager::get().put_other_changes(); }

void clear_other_changes(bool backup) { _BBS_Backup_Manager::get().clear_other_changes(backup); }

bool has_other_changes(bool backup) { return _BBS_Backup_Manager::get().has_other_changes(backup); }

SaveObjectGaurd::SaveObjectGaurd(ModelObject& object) { _BBS_Backup_Manager::get().push_object_gaurd(object); }

SaveObjectGaurd::~SaveObjectGaurd() { _BBS_Backup_Manager::get().pop_object_gaurd(); }
...
/// </summary>
const TextConfigurationSerialization::TypeToName TextConfigurationSerialization::type_to_name = boost::assign::list_of<TypeToName::relation>(
    EmbossStyle::Type::file_path, "file_name")(EmbossStyle::Type::wx_win_font_descr,
...
FontProp::HorizontalAlign read_horizontal_align(const char**                                                 attributes,
                                                unsigned int                                                 num_attributes,
                                                const TextConfigurationSerialization::HorizontalAlignToName& horizontal_align_to_name)
{
    ...
    if (horizontal_align_str.length() == 1) {
        ...
        if (boost::spirit::qi::parse(horizontal_align_str.c_str(), horizontal_align_str.c_str() + 1, boost::spirit::qi::int_,
                                     horizontal_align_int))
...
FontProp::VerticalAlign read_vertical_align(const char**                                               attributes,
                                            unsigned int                                               num_attributes,
                                            const TextConfigurationSerialization::VerticalAlignToName& vertical_align_to_name)
{
    ...
    if (vertical_align_str.length() == 1) {
        ...
        if (boost::spirit::qi::parse(vertical_align_str.c_str(), vertical_align_str.c_str() + 1, boost::spirit::qi::int_,
                                     vertical_align_int))
...
std::optional<TextConfiguration> TextConfigurationSerialization::read(const char** attributes, unsigned int num_attributes)
{
    ...
    fp.align = FontProp::Align(read_horizontal_align(attributes, num_attributes, horizontal_align_to_name),
    ...
    if (collection_number > 0)
        fp.collection_number = static_cast<unsigned int>(collection_number);
...
bool to_xml(std::stringstream& stream, const EmbossShape::SvgFile& svg, const ModelVolume& volume, mz_zip_archive& archive)
{
    ...
    return mz_zip_writer_add_mem(&archive, svg.path_in_3mf.c_str(), (const void*) file_data_str.c_str(), file_data_str.size(),
                                 MZ_DEFAULT_COMPRESSION);
}
